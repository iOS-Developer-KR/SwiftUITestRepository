...
//    }
    ...
    func getAllProducts(pricedescending descending: Bool?, forCategory category: String?, count: Int, lastDocument: DocumentSnapshot?) async throws -> ([Product], lastDocumenet: DocumentSnapshot?) {
    ...
    func  getAllProductsCount() async throws -> Int {
        // getAggregation: 서버에서 document의 개수를 계산해서 bytes로 전송하여 데이터 효율적이면서 좋다
        try await productsCollection
            .aggregateCount()
    }
